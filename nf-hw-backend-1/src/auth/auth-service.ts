...
class AuthService {
  ...
  async registerUser(createUserDto: CreateUserDto): Promise<IUser> {
    ...
    if (!email || !password || !username || !city) {
      console.error('Missing fields:', { email, password, username, city });
      throw new Error('All fields must be provided (email, password, username, city)');
    }
  ...
  private generateJwt(user: IUser): string {
    try {
      return jwt.sign({ id: user._id, email: user.email, city: user.city }, this.jwtSecret, { expiresIn: '15m' });
        ...
        console.error('Ошибка при создании JWT:', err);
        throw err;
    }
  }
  ...
  private generateRefreshToken(user: IUser): string {
    try{
      ...
      console.error('Ошибка при создании JWT Refresh:', err);
      throw err;
  }
