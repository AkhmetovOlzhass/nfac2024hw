...
const UserDetails = () => {
  ...
  const toggleFavorite = async (songId) => {
    try {
      const response = await axios.put(`https://nfac2024hw.vercel.app/api/v5/songs/${artistId}/favorite`, { songId });
  ...
  const onPlaylistSelect = async (playlistId, songId) => {
    try {
      const response = await axios.put(`https://nfac2024hw.vercel.app/api/v5/songs/${playlistId}/add-song`, { songId });
      if (response.status === 200) {
  ...
  const handleDeleteClick = async (songId) => {
    try {
        await axios.delete(`https://nfac2024hw.vercel.app/api/v5/songs/${songId}`);
